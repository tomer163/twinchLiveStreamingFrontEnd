<template>
    <div class="h-16 flex w-full bg-gray-900 items-center px-3">
        <div class="text-xl font-medium flex items-center">
            ...
            <RouterLink v-if="currentLoggedUser !== ''" v-for="route in routes.filter(route => route.path !== '/')" :to="route.path" class="mx-6">{{ route.name }}</RouterLink>
        </div>
        <div class="grow"></div>
        <div v-if="currentLoggedUser !== null">
...
const currentLoggedUser = ref(null)
